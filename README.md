...
The org's internal dev toolkit: agents, skills, and instructions for **development, PR review, testing, and SDLC process** — generated for GitHub Copilot CLI, Claude Code, and Gemini CLI at once, from one install command. It also maintains **living, per-repo principles docs** (architecture/design/business domain) that agents keep in sync as the codebase evolves. Security, SRE, architecture governance, and PM/Jira/Confluence tooling are not covered yet.
...
Rather than installing everything blind, run the **`repo-init`** agent from inside the target repo (via whichever of the three CLIs you use — e.g. `/agent repo-init` in Copilot CLI). It scans the repo first, figures out what's actually relevant (which CLIs are in use, whether test tooling exists, what stack this is), and then runs `bin/toolkit` with a sensible selection instead of installing every agent and skill unconditionally. It also bootstraps `principles/` for you as part of the same run — see below.

## Principles: living per-repo documentation

Unlike agents/skills/instructions, `principles/` isn't org-wide content copied into every repo — it's specific to *this* repo, generated by reading the actual code, and kept current as the code changes:

| File | What it documents |
|---|---|
| `principles/architecture.md` | This repo's actual system structure — major components, how they interact. |
| `principles/design.md` | Design/code conventions specific to this repo, beyond the generic `dev-scaffolding` skill. |
| `principles/domain.md` | The business domain model — entities, terminology, rules. Skipped for repos with no real business domain (e.g. a pure library). |

**Generating them for the first time:**
- If the repo already has substantial code, run the **`principles`** agent (e.g. `/agent principles`) — it reads the codebase and derives real content.
- If the repo is new with little or no code yet, scaffold placeholders instead:

**Scaffold placeholders:**
```
./bin/toolkit scaffold-principles
```

**Keeping them current:** this is the point of the feature, not an afterthought. The `dev` agent updates the relevant `principles/*.md` file as part of any change that affects architecture, design, or the domain model — not as a separate task. The `pr-review` skill treats a PR that changes one of those things without a matching principles update as a **blocking** finding, the same severity as a missing test.

**Important:** `principles/*.md` is never touched by `update`, `status`, or `uninstall` — it isn't manifest-tracked at all, because there's no org-wide source to sync it against. It's yours (and your agents') to maintain directly, like any other file in the repo.
...
| `repo-init` | Bootstraps the toolkit into a new repo — scans first, then installs only what's relevant, then bootstraps `principles/`. Run this one first in a new project. |
| `principles` | Generates or refreshes `principles/architecture.md`, `design.md`, `domain.md` by reading the actual codebase. |
...
| principles | `principles-maintenance` | What belongs in each `principles/*.md` file and how the `dev`/`reviewer` agents keep them current. |
...
- `scan`'s "substantial code" signal (used to decide whether to scaffold placeholder principles or recommend the `principles` agent) is a rough file-count/line-count heuristic, not a code quality measure — it can be wrong for unusual repo layouts.
- Security, SRE, architecture governance, and PM/Jira/Confluence tooling are out of scope for this version — they'll come later as their own skill areas.
